...
class DataModuleImporter(ModuleImporter):
    ...
    def _register_route(self, data_provider, method_name, endpoint, method):
        ...
        if callable(getattr(data_provider.__class__, method_name, None)):
            ...
            rule = f"{endpoint if method_name != 'execute' else self.execute_path}/<subpath>" if use_subpath else endpoint
            view_func = lambda subpath=None: route(subpath) if use_subpath else route()
            self.app.add_url_rule(
                rule,
